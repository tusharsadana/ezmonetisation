...
class DashboardService:
    ...
    @staticmethod
    async def watch_hour_earn_graph(session: AsyncSession, user_email: str, time_period: TimePeriodEnum):
        query = user_exists(user_email)
        result = await session.execute(query)
        result = result.one_or_none()
        if not result:
            return False, "Invalid user_email"
        query = user_in_watch_credit(user_email)
        result = await session.execute(query)
        result = result.one_or_none()
        if not result:
            return False, "User is yet to earn and consume any watch hours"
        query = watch_hour_log(user_email, time_period)
        result = await session.execute(query)
        data = result.all()
        if not len(data):
            return False, "No user activity in the given time period"
        ...
        if len(positive_values):
            plt.plot_date(positive_x, positive_y, '-o', label='Watch Hours Earned')
        if len(negative_values):
            plt.plot_date(negative_x, negative_y, '-o', label='Watch Hours Consumed')

        plt.xlabel('Time')
        plt.ylabel('Watch Hours')
        plt.title('Cumulative Watch Hours')
        plt.legend()
        plt.grid(True)
